...
<style>
    ...
    .deletebtn{
        background: url(<?php echo plugins_url('/elex-woocommerce-dynamic-pricing-and-discounts/img/del.png'); ?>) 10px 10px no-repeat;
        width: 15px;
        height: 15px;
        background-size: 100%;
    ...
    .editbtn{
        background: url(<?php echo plugins_url('/elex-woocommerce-dynamic-pricing-and-discounts/img/edit.png'); ?>) 10px 10px no-repeat;
        width: 15px;
        height: 15px;
        background-size: 100%;
        background-position: top left;
...
<!--        <a href="?page=dynamic-pricing-main-page&tab=licence" class="nav-tab  thickbox <?php //echo $active_tab == 'import_export' ? 'nav-tab-active' : ''; ?>"><?php //_e('Import/Export', 'eh-dynamic-pricing-discounts'); ?></a>  -->
        <a href="?page=dynamic-pricing-main-page&tab=licence" style="color:red;" class="nav-tab <?php echo $active_tab == 'licence' ? 'nav-tab-active' : ''; ?>"><?php _e('Go Premium', 'eh-dynamic-pricing-discounts'); ?></a>
...
//                $delimiter = ($key < count($execution_order) - 1) ? '  |  ' : ' ';
    ...
    <div id="col-container2">
        <div class="col-wrap">
            <div class="inside">
                <div style="">
                    <button style="margin: 10px 0px;<?php if (!empty($_REQUEST['edit']) || in_array($active_tab,array('settings_page','import_export','licence','No Rules Selected'))) echo "display:none;" ?> <?php echo $active_tab; ?>" class="add_new add_new_rule_btn button button-primary" onclick="elex_dp_show_form()" >Add New Rule</button> </div>
                ...
                ?>
                    ...
                    if (isset($_REQUEST['tab']) && !empty($_REQUEST['tab'])) {
                        echo sanitize_text_field($_REQUEST['tab']);
                    } else {
                        echo $active_tab;
                    }
                    ?>">
                    ...
                    if ($active_tab == 'category_rules') {
                        ...
                        require('selected-tab-content/elex-settings_page-tab.php');
                    ...
                    if ($active_tab == 'import_export' && !empty($_REQUEST['action']) && sanitize_text_field($_REQUEST['action']) == "delete_all_rules") {
                        if (is_admin()) {
                    ...
                    if ($active_tab != 'import_export' && $rule_tab) {
                        ...
                        ?>" id="cancel_btn" class="button button-primary" <?php if(!isset($_REQUEST['edit'])){echo ' onclick="return elex_dp_hide_rule_form()" ';} ?> >Cancel</button>
                        <?php } ?>
                    </p>
                </form>
                ...
                <form name="alter_display_form" method="get" action="">			<!--Displays Table List Of Saved Rules--->
                    ...
                    ?>">
                           ...
                           if (isset($_REQUEST["tab"]) && !empty($_REQUEST["tab"])) {
                               if (file_exists(plugin_dir_path(__FILE__) . '/selected-tab-content/elex-display-saved-' . $_REQUEST["tab"] . '-table.php') === true)
    ...
    function elex_dp_update_rules_arrangement() {
        ...
        jQuery.post(
                ajaxurl,
                {
                    ...
                    'rules-type': '<?php echo !empty($_REQUEST['tab'])? sanitize_text_field($_REQUEST['tab']) :current($execution_order); ?>',
...
<style>
    td.icon-move{
        background-image: url('<?php echo plugins_url('elex-woocommerce-dynamic-pricing-and-discounts/img/drag2.png'); ?>');
