...
class WCCS_Cart_Discount {
	...
	public function get_possible_discounts() {
        ...
        foreach ( $valids as $discount ) {
            ...
            if ( 'percentage' === $discount->discount_type ) {
                $discount_amount = $discount_amount * ( $cart_subtotal / 100 );
            }
	...
	protected function get_cart_subtotal() {
		$subtotal = 'yes' === get_option( 'woocommerce_prices_include_tax' ) ?
			$this->cart->subtotal : $this->cart->subtotal_ex_tax;

		return apply_filters( 'wccs_cart_discount_get_cart_subtotal', $subtotal );
	}
