    ...
    protected function get_onsale( $product, $rules, $type ) {
        ...
        if ( ! $onsale && 'variable' === $product->get_type() ) {
            ...
            foreach ( $varations as $variation ) {
                ...
                if ( $this->pricing->is_in_exclude_rules( $product->get_id(), $variation['variation_id'], array() ) ) {
                    continue;
                }
        ...
        set_transient( $transient_name, ( $onsale ? 'yes' : 'no' ), DAY_IN_SECONDS * 30 );
